...
function populateWeatherInfo(data) {
    ...
    const [{ description, id, icon}] = weather; 
    const { speed } = wind; 
    document.querySelector('#weather-icon').src= `https://openweathermap.org/img/wn/${icon}@2x.png`
    document.querySelector('.city').textContent = name;
    document.querySelector('.current-state').innerHTML=description;
    document.querySelector('.humidity').firstChild.textContent=humidity;
    document.querySelector('.pressure').firstChild.textContent=pressure;
    document.querySelector('.temp').firstChild.textContent=temp;
    document.querySelector('.wind').firstChild.textContent=speed;
